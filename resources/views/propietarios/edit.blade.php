<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <a href="{{ route('propietarios.edit') }}"  style="color: rgb(111, 111, 111);">Editar propietario: {{ $propietario->name }} {{ $propietario->surname }}</a>
        </h2>
    </x-slot>
	@if (session('status'))
    ...
    <form method="POST" action="{{ route('propietarios.update',  $cliente->id) }}" class="mx-40" style="min-width: 40%;">
        @csrf
        @method('PUT')
        {{-- Nombre --}}
        <div class="mb-4">
