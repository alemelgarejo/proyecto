<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <a href="{{ route('ordenes.create') }}"  style="color: rgb(111, 111, 111);">Crear órden</a>
        </h2>
    </x-slot>
	@if (session('status'))
    ...
    <form method="POST" action="{{ route('ordenes.store') }}" class="mx-40" style="min-width: 40%;">
        @csrf
        @method('POST')
        ...
        <div class="mb-4">
            ...
            <select class="form-control" name="cliente_id" id="cliente_id">
                @foreach ($clientes as $name => $id)
                    <option {{ $ordene->cliente_id == $id ? 'selected="selected"' : '' }} value="{{ $id }}">
                        {{ $name }}</option>
                @endforeach
            </select>
            @error('cliente_id')
        ...
        <div class="mb-4">
            ...
            <input name="fecha" type="date" placeholder="Fecha" class="form-control" id="fecha" aria-describedby="fecha"
    ...
    <div class="mb-4">
     ...
     <div class="form-check">
         <input class="form-check-input" type="radio" name="acceso_minusvalidos" id="acceso_minusvalidos_no" value="0" @if ($ordene->acceso_minusvalidos == 0) checked @else @endif>
		 <label>
             No
         </label>
     </div>
 ...
 <hr>
        ...
        <div class="mb-4">
            ...
            <select name="estado_conservacion" placeholder="Estado de Conservacion" id="estado_conservacion" class="form-control" value="{{ old('estado_conservacion', $ordene->estado_conservacion) }}">
                ...
                <option value="Antiguo" {{ $ordene->estado_conservacion == 'Antiguo' ? 'selected' : '' }}>Antiguo</option>
